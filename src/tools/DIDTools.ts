...
import {DIDDocument, ServiceName, VCTemplateSectionTitle} from './DIDInterfaces';
...
export const tonClient = new TonClient({network: {endpoints: ['main.ton.dev']}});

const DIDStorageContractAddress = '0:256dfbf328016c5622aee36d2098b829aa06fe99b37bba9d5ca588e6e303d9f9';

export class DIDTools {

    static async loadDIDDocument(did: string): Promise<DIDDocument | null> {
        ...
        try {
            const response = await acc.runLocal('getDid', {id: `0x${did}`});
            log.info('LOADED DID DOCUMENT', response.decoded?.output.value0.didDocument);
            return JSON.parse(response.decoded?.output.value0.didDocument);
        } catch (err) {
            log.warn('DID Document load failed', err);
            return null;
        }
    }
    ...
    static async initDB(prisma: PrismaClient): Promise<void> {
        ...
        await prisma.service.upsert({
            ...
            update: {
                ...
                description: 'TON Swap is a decentralized exchange (DEX) that allows users to carry out DeFi operations with Free TON tokens. Provide verifiable credentials (VCs) to get special offers from TON SWAP',
        ...
        await prisma.service.upsert({
            ...
            update: {
                ...
                description: 'Easy vote is a primary tool for Free TON Jury that provides functionality for submissions analysis and contests voting.',
        ...
        await prisma.vCTemplate.upsert({
            ...
            update: {
                ...
                title: 'Proof of TON address',
                description: 'Prove the ownership of Free TON blockchain address with a verifiable credential. This credential in necessary to ensure you’ve been an Free TON network user.',
