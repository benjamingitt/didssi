...
import {CredentialSubject, VerifiableCredentialEd25519} from '../tools/DIDInterfaces';
...
const mutation: Resolvers = {
    Mutation: {
        ...
        loginGenerate: async (parent, {did}, {prisma}) => {
            ...
            const message = `Please sign this data to login: ${ AuthUtils.generateToken()}`;
        ...
        loginVerify: async (parent, {did, signatureHex}, {prisma, request}) => {
            ...
            if (!DIDDoc) {
                throw new GraphQLError({message: 'DID Document not found', code: StatusCodes.NOT_FOUND});
            }
        ...
        vcCreateProofOfTonAddressVerify: async (parent, {signatureHex, walletAddress}, {session, prisma}) => {
            ...
            const didDoc = await DIDTools.loadDIDDocument(session.account.did);
            if (!didDoc) {
                throw new GraphQLError({message: 'DID Document not found', code: StatusCodes.NOT_FOUND});
            }
            ...
            if (walletCustodians?.indexOf(`0x${session.account.did}`) === -1) {
                throw new GraphQLError({
            ...
            const credentialSubject: CredentialSubject = {
                id: `did:freeton:${session.account.did}`,
                degree: {
            ...
            const vcSignature = await DIDTools.signMessage({
                ...
                privateKey: config.freeTonIdKeys.secret
            });

            const VC: VerifiableCredentialEd25519 = {
                ...
                proof: {
                    ...
                    created: new Date(),
                    verificationMethod: `did:freeton:${config.freeTonIdKeys.public}#${session.account.did}`
                },
                credentialSubject,
                issuanceDate: new Date(),
                issuer: {
                    id: `did:freeton:${config.freeTonIdKeys.public}`
                }
            };
